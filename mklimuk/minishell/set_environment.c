...
// we need a garbage container or we'll get memory leaks. because we take memory for ft_strdup, ft_substr and so on 
...
// the difference is that environment variable is a massive of strings, but envlist variable is a list with keys and values. 
